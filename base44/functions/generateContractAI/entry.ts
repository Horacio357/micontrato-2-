...
import comercialV9Json from './templates/comercial-v9.ts';
import viviendaV9Json from './templates/vivienda-v9.ts';
import temporariaV9Json from './templates/temporaria-v9.ts';

// ============================================================================
// MOTOR DE PLANTILLAS ESTRICTAS (100% DETERMINISTA — SIN IA)
// ----------------------------------------------------------------------------
// Los contratos NO se generan con un modelo de lenguaje. Cada contrato es una
// plantilla fija, estandarizada por un abogado, definida en STRICT_TEMPLATES
// o en los archivos JSON locales de base44/templates/.
//
// Reglas de oro (document_settings del abogado):
//   - allow_markdown: false  -> NUNCA se emite Markdown, negritas (**) ni #.
//   - allow_bold_text: false -> texto plano puro (el PDF lo justifica).
//   - Los bloques se concatenan según "rules". Cero interpretación libre.
//
// Nota de arquitectura: las funciones backend NO admiten imports locales, por
// lo que todo el motor vive inline en este archivo.
// ============================================================================
...
// El texto de la plantilla y cada valor del formulario son jurídicamente
// significativos. El motor no capitaliza, limpia ni reescribe contenido: solo
// sustituye marcadores declarados por el abogado.

// --- Conversor de números a letras (es-AR) --------------------------------
// Soporta {{escribir_en_letras(campo)}} del template V6, evitando campos _letras.
...
// Convierte un valor numérico (admite punto/coma decimal) a letras en mayúsculas.
function escribirEnLetras(rawValue, isRate = false) {
  if (rawValue === undefined || rawValue === null || rawValue.toString().trim() === "") {
    return "___________";
  }
  // Normaliza: quita separadores de miles, unifica decimal a punto.
  let s = rawValue.toString().trim().replace(/\s/g, "");
  // Si tiene coma y punto, la coma es decimal (formato AR: 1.000.000,50)
...
// ============================================================================
// PLANTILLA: MUTUO ONEROSO (V9_Monolitico_Literal)
// ----------------------------------------------------------------------------
// Copia literal del contrato del abogado: un único bloque de texto íntegro,
// sin cláusulas condicionales ni reescritura. El motor solo reemplaza las
// variables {{...}} — el resto del texto nunca se modifica.
// ============================================================================
const MUTUO_ONEROSO = {
  // Mapeo: clave anidada del template -> posibles claves planas del wizard.
  field_map: {
...
// ============================================================================
// PLANTILLA: LOCACIÓN DE SERVICIOS PROFESIONALES
// V9_Monolitico_Literal_Final — texto íntegro provisto por el abogado.
// ============================================================================
...
// ============================================================================
// PLANTILLA: CONVENIO DE CONFIDENCIALIDAD
// V9_Monolitico_Literal_Final — texto íntegro provisto por el abogado.
// ============================================================================
...
function getV9Template(document) {
  ...
  const templateVariables = [...templateText.matchAll(/\{\{\s*(?:escribir_en_letras\()?\s*([\w.]+)/g)].map((match) => match[1]);
  const unmappedVariables = [...new Set(templateVariables)].filter((key) => !field_map[key]);
  const unusedSchemaFields = Object.keys(field_map).filter((key) => !templateVariables.includes(key));
  if (unmappedVariables.length || unusedSchemaFields.length) {
    throw new Error(`Plantilla V9 inconsistente. Variables sin mapeo: ${unmappedVariables.join(", ") || "ninguna"}. Campos sin uso: ${unusedSchemaFields.join(", ") || "ninguno"}.`);
  }
  return {
    field_map,
    required_inputs,
...
// El motor no detecta nombres propios automáticamente: se declara por campo.
// "first" = solo la primera letra del valor (nombres de persona).
// "title" = tipo oración por palabra, excluyendo partículas (topónimos).
...
function interpolate(template, formData, fieldMap) {
  // El texto de la plantilla es literal: no se normaliza espaciado ni se
  // reescribe el contenido estático. Solo se sustituyen las variables.
...
async function buildStrictContract(slug, formData) {
  ...
  // Solo varían los bloques expresamente declarados en las reglas maestras;
  // el resto del contrato permanece literal e inmutable.
  ...
  if (slug === "locacion-vivienda") {
    const quantity = String(formData.garantes_cantidad || "No aplica");
    const guarantorPattern = /DECIMO SEXTA \(garantes\):[\s\S]*?(?=\n\nDECIMO SEPTIMA)/;
    if (quantity === "No aplica") {
      templateText = templateText.replace(guarantorPattern, "DECIMO SEXTA (garantes): Las partes acuerdan que el presente contrato se celebra sin la constitución de garantes ni fiadores.");
    } else {
      const person = (number) => `el/la Sr./Sra. {{garantes.g${number}_nombre_opcional}}, D.N.I. N° {{garantes.g${number}_dni_opcional}}, C.U.I.L./C.U.I.T. N° {{garantes.g${number}_cuit_cuil_opcional}}, de nacionalidad {{garantes.g${number}_nacionalidad_opcional}}, de estado civil {{garantes.g${number}_estado_civil_opcional}}, con domicilio en {{garantes.g${number}_domicilio_calle_opcional}}, ciudad de {{garantes.g${number}_ciudad_opcional}}, departamento/partido de {{garantes.g${number}_departamento_opcional}}, provincia de {{garantes.g${number}_provincia_opcional}}, correo electrónico {{garantes.g${number}_correo_electronico_opcional}}`;
      const people = quantity === "2" ? `${person(1)}; y ${person(2)}` : person(1);
      const obligation = quantity === "2"
        ? "se constituyen en codeudores solidarios, lisos, llanos y principales pagadores"
        : "se constituye en codeudor solidario, liso, llano y principal pagador";
      templateText = templateText.replace(guarantorPattern, `DECIMO SEXTA (garantes): ${people}, ${obligation} de todas las obligaciones emergentes del presente contrato, renunciando expresamente a los beneficios de división y excusión.`);
    }
  }
...
export default async function(req) {
  try {
    ...
    const { contractSlug, province, formData, preview = false } = await req.json();
    if (!contractSlug || !province || !formData) {
      return Response.json({ error: "Faltan datos requeridos: contractSlug, province, formData" }, { status: 400 });
    }

    const contract = await buildStrictContract(contractSlug, formData);
    if (!contract) {
      return Response.json({ error: `No se encontró una plantilla estricta para "${contractSlug}".` }, { status: 404 });
    }
